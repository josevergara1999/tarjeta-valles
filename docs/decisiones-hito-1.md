...
| Duración de cada pase | **En `settings`** (31-ago-2026): `dias_pase_dia=1`, `dias_pase_3=3`, `dias_pase_7=7`, `dias_pase_14=14`. El spec 3.3 pedía `fecha_expiracion = fecha_activacion + N días` pero el N no estaba escrito en ninguna parte: vivía en el nombre del producto. La clave se arma como `dias_` || tipo, así que un pase nuevo en el Hito 2 es una fila más. Las suscripciones no están: arrancan al pagar. |
| Pase que vence entre reservar y validar | **Se valida igual** (31-ago-2026). Un turista que reserva a las 23:58 con el pase venciendo a medianoche y al que el mesón teclea el código a las 00:03 no pierde el canje: hizo su parte a tiempo y rechazarlo sería castigarlo por la lentitud del local, delante del cajero. El código vive 5 minutos, así que la ventana de abuso es mínima. |
